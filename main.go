...
func main() {
	cmd := &cobra.Command{
		Use:   "spank-omen",
		Short: "Plays audio responses when laptop impact events are detected",
		Long: `spank-omen plays audio responses when a slap or hit is detected.

Current backend: Apple Silicon accelerometer via IOKit HID.
Planned backend: Windows microphone impact detection and audio classification.
...
within a minute, the more intense the sounds become.`,
	...
	cmd.Flags().BoolVar(&fastMode, "fast", false, "Enable lower-latency detection tuning")
...
func run(ctx context.Context, tuning runtimeTuning) error {
	if os.Geteuid() != 0 {
		return fmt.Errorf("spank-omen requires root privileges for accelerometer access, run with: sudo spank-omen")
	}
